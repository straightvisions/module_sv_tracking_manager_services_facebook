<?php
	...
	class facebook extends modules {
		public function init() {
			// Section Info
			$this->set_section_title( __('Facebook', 'sv_tracking_manager' ) )
				 ->set_section_desc(__( sprintf('%sFacebook Ads Login%s', '<a target="_blank" href="https://www.facebook.com/adsmanager">','</a>'), 'sv_tracking_manager' ))
		...
		protected function load_settings(): facebook {
			...
			$this->get_setting('tracking_id')
				 ...
				 ->set_description( __( sprintf('%sHow to retrieve Tracking ID%s', '<a target="_blank" href="https://developers.facebook.com/docs/facebook-pixel/">','</a>'), 'sv_tracking_manager' ) )
				 ->load_type( 'text' );
			return $this;
		}
		protected function register_scripts(): facebook {
			if($this->is_active()) {
